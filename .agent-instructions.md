...
För produktens mål och pedagogiska avvägningar gäller det antagna [produktkontraktet](docs/PRODUCT_CONTRACT.md) över detta dokument. I övrigt gäller detta dokument framför andra filer i `docs/`. Verktygs-, säkerhets- och behörighetsregler påverkas inte av produktkontraktet.

Produktkontrakt v1.0 antogs av Simon 2026-09-20. Det beskriver avsedd funktion, inte verifierad leverans.
Källjämförelse och beslut finns i [docs/PRODUCT_CONTRACT_REVIEW.md](docs/PRODUCT_CONTRACT_REVIEW.md);
elev–lärarscenarier och underlag för klassobservation finns i [docs/PRODUCT_SCENARIOS.md](docs/PRODUCT_SCENARIOS.md).
Funktionskedjans aktiva tekniska baslinje finns i [docs/FUNCTION_CONTRACTS.md](docs/FUNCTION_CONTRACTS.md).
Vid motsägelse ska avvikelsen registreras och rättas mot kontraktet, inte döljas genom att anpassa produktmålet till befintlig kod.
...
export default {
  ...
  verifyContent(problem) → { valid, reason },
...
**`verifyContent`** räknar fram facit oberoende från generatorns lagrade
`answer`. Körningsguardian använder kontrollen före visning. Verifieraren ska
läsa uppgiftens strukturerade värden eller uttryck, inte återanvända samma
facitfält eller generatorberäkning som den granskar.
...
3. getDomain(domain).verifyContent(problem) → oberoende facitkontroll
4. visa via getDomain(problem.domain).Display
5. eleven svarar
6. getDomain(problem.domain).evaluate(problem, answer) → evalResult
7. getDomain(problem.domain).analyzeError(problem, answer) → errorAnalysis
8. updateProfile(profile, problem, evalResult, errorAnalysis)
9. tillbaka till 1
```
...
### R2. Sammanhållna filer

Filstorlek är en granskningssignal, inte en hård kvalitetsgräns. En fil kan vara längre än 400 rader när den har ett sammanhållet ansvar, till exempel en mallkatalog eller en avgränsad generator.

Dela upp en fil när den blandar flera ansvar, har delar som förändras oberoende av varandra, eller blir svår att testa och förstå. Sträva efter små funktioner och komponenter med tydliga ansvar. Vid cirka 600 rader ska nästa ändring bedöma om en uppdelning är motiverad och dokumentera beslutet i commit eller PR-beskrivning.
