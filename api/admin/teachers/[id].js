...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'DELETE') {
      ...
      if (currentRole === SUPER_ADMIN_ROLE) await mutateProtectedSuperAdmin(id, null)
      else {
        await kv.del(key)
    ...
    if (currentRole === SUPER_ADMIN_ROLE && updated.role !== SUPER_ADMIN_ROLE) {
      await mutateProtectedSuperAdmin(id, updated)
    } else {
