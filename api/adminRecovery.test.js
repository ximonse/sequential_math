...
describe('temporary admin recovery', () => {
  beforeEach(() => {
    ...
    records.set('teacher_account:admin', { id: 'admin', isAdmin: false, sessionVersion: 3, passwordHash: hash, passwordSalt: salt, passwordScheme: scheme })
  ...
  it('requires an explicit account selection when multiple teacher accounts are active', async () => {
    const { hash, salt, scheme } = hashTeacherPassword('other-password')
    records.set('teacher_accounts:index', ['admin', 'other'])
    records.set('teacher_account:other', { id: 'other', isAdmin: false, sessionVersion: 1, passwordHash: hash, passwordSalt: salt, passwordScheme: scheme })
