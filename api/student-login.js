...
import { isStudentDeleted, mutateStudentRecord } from './_studentStore.js'
...
import { createStudentSession, issueStudentSession, setStudentSessionCookie, studentIdentityDto, verifyPilotStudentPin } from './_studentSession.js'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const qrPinLogin = profile?.auth?.scheme === 'qr-pin-v1'
    const credentialsValid = qrPinLogin
      ? verifyPilotStudentPin(profile.auth, code)
      : verifyPasswordAgainstAuth(profile?.auth, code)
    if (!profile || !credentialsValid) {
    ...
    await mutateStudentRecord(profile.studentId, current => ({ ...current, auth: { ...current.auth, failedCodeAttempts: 0, lastFailedCodeAt: null, lastLoginAt: Date.now(), loginCount: Number(current.auth?.loginCount || 0) + 1 } })).catch(() => undefined)
    if (qrPinLogin) {
      const session = await createStudentSession(profile)
      setStudentSessionCookie(res, session.id)
      return res.status(200).json({ ok: true, student: studentIdentityDto(profile), csrfToken: session.csrfToken,
        classId: classRecord.id, className: classRecord.name })
    }
    // Transitional compatibility for pupils that have not received a QR/PIN card yet.
    const sessionSecret = await issueStudentSession(profile.studentId, req.body?.remember === true)
    return res.status(200).json({ studentId: profile.studentId, sessionSecret, classId: classRecord.id,
      className: classRecord.name, legacySession: true })
  } catch {
