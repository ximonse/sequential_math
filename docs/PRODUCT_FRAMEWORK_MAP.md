...
**Nästa avgränsade arbete: E3:s `CurrentNeed` och flerledade återhämtning.** Låt samma beslutsmodell äga `introduce`, `consolidate`, `challenge`, `recover` och `support` i stället för att mastery-golv, förmågeskattning och felstreak konkurrerar. Återspelbara S2–S4 ska först visa nuvarande motexempel och därefter att ett ensamt rätt svar inte avslutar återhämtning. Nya pedagogiska trösklar kräver Simons beslut; struktur, spårbarhet och regressionsfall gör inte det. Publicering kräver separat begäran.
