...
Status: **kartläggning v0.8, 2026-09-21; E1 kontrakterat, E2 lokalt implementerat och E3:s sammanhängande beslutskedja lokalt implementerad**.
...
| F2.1 Nästa träningsbeslut — P2/P3/P9, D1/D2 | Ett ansvar fattar nästa beslut från aktuellt behov inom F1. Historiskt belagt kunnande är separat. | `sessionStartDecision`, `currentNeed`, `scopedSelection`, `adaptiveEngine`, sessionsregler | Scoped urval läser versionsmärkta beslut för introduktion, warmup, befästande, utmaning, flerledad återhämtning och support. Den parallella förmågeskattningen finns kvar för legacyval och behöver nu avgränsas eller tas bort som pedagogisk auktoritet. | S2/S3/S4/S7: långa förlopp, långsamma rätt, ihållande fel och återkomst. |
| F2.2 Avancering och bekräftelse — P3/P7, D1 | Automatisk progression; eventuell gratulation efter belagt kunnande, utan elevval. | `adaptationDecision`, `addProblemResult`, `usePracticeCoreActions`, `ProgressionMilestoneOverlay` | Masteryövergången har en versionsmärkt ägare och ja/nej-flödet är borttaget. Startbeslut och aktuell behovsstyrning konsumeras nu av samma scoped urval. | S2/S7: nästa uppgifter och bekräftelse följer samma beslut, även efter omladdning. |
...
| E5 Innehåll och samlad kvalitetsgrind — **tre kodsnitt lokalt implementerade 2026-09-21** | Körningsguardian kasserar kandidater med fel domän, kompetens, nivå, visning, svarstyp, facit eller evidens och stoppar efter begränsade omförsök. Återläsningen skiljer kontraktsgiltiga masteryfakta från bevarad legacyhistorik. Oberoende facitkontroll och variationsgrind täcker nu alla registrerade kompetenser/steg; samlad UI-/klassrumsverifiering återstår. | Börjar med motexempel i E1/E2, avslutas efter E3/E4; innehållsfel P0 | S1–S10 har daterat resultat och begränsningar. Tester/build samt relevanta browser-/iPad-flöden är kontrollerade. Ingen automatisk publicering. |
...
Den lokala cutover-policyn bevarar därför legacyfakta men ger bara ett faktum självständig auktoritet när regelversion och konkreta observations-ID:n finns. Säkert klassificerbara råobservationer kan fortfarande räknas om. Oklassificerbart historiskt underlag blir obelagt i stället för att tyst fortsätta styra eleven; ingen lagrad elevpost raderas av denna policy.
...
- `support` och ett gemensamt start/warmupbeslut återstod efter detta snitt.
...
### E3 — lokalt verifierat tredje snitt 2026-09-21

- Efter fyra observationer i aktiv återhämtning utan två fullständigt rätta svar går `CurrentNeed` till `support` på samma klampade målnivå.
- Eleven pausas inte och får ingen automatisk stöddialog. Scoped urval fortsätter använda målnivån.
- Lärarsammanfattningen skapar en spårbar felsignal från observationernas ID och visar upp till sex faktiska frågor, elevsvar och facit tillsammans med innehåll och nivå.
- Signalen är ett underlag för lärarens beslut, inte en diagnos eller automatisk tilldelning. Två fullständigt rätta svar i följd återgår till `consolidate` och tar bort signalen vid nästa sammanställning.

Verifiering: `npm run test` passerade 68 testfiler/307 tester, inklusive rendering av felsvaret i lärarpanelen. `npm run build` passerade med de befintliga Browserslist-/bundlevarningarna och avgränsad ESLint passerade. Ingen autentiserad browser-, iPad-, live- eller produktionsverifiering gjordes.

### E3 — lokalt verifierat fjärde snitt 2026-09-21

- `sessionStartDecision` äger nu både fokuserad sessionsramp och warmup efter frånvaro enligt regelversion 1.
- Första träning i en kompetens får syftet `introduce`; återkomst och fokuserad ramp med historik får `consolidate`. Besluts-ID, regelversion, reason codes, ram och målnivå följer den genererade uppgiften till observationen.
- Frånvarowarmup använder de befintliga två till fyra uppgifterna och nivåavlastningen, men det tidigare 70/30-slumpvalet är borttaget. Dagens svar räknas utan att det ursprungliga frånvarounderlaget försvinner efter första uppgiften.
- `recover` och `support` har företräde framför start/warmup. Den gamla felstreakssänkningen läggs inte längre en extra gång ovanpå ett versionsmärkt behov.
- Fokuserad startplan fryses för sessionen och klampas inom en eventuell lärarlåst ram.

Verifiering: `npm run test` passerade 69 testfiler/315 tester, avgränsad ESLint passerade och `npm run build` passerade med de befintliga Browserslist-/bundlevarningarna. Ingen autentiserad browser-, iPad-, live- eller produktionsverifiering gjordes.

### E4 — lokalt verifierat första snitt 2026-09-21

- Lärarraden exponerar nu `attainmentLevels` från den gemensamma lärarsammanfattningen och `currentNeeds` från den versionsmärkta beslutsmodellen som skilda fält. Otränat/obelagt blir `null`, inte nivå 1.
- Elevdetaljens tidigare "Nivå per räknesätt" visar nu "Belagd nivå / tränar nu". Den flytande legacyförmågan och historiskt högsta globala värdet visas inte som kunnande.
- Snabbuppdrag väljer nivå från faktiska felsvar, aktuellt behov eller nästa steg efter belagd nivå. `operationAbilities`/`currentDifficulty` är inte längre fallback.
- Träningsprioriteringen avgränsar sin nivåhorisont med observerade nivåer, belagd mastery och aktuellt behov, inte en legacyförmåga upp till nivå 12.
...
### E5 — lokalt verifierat andra kodsnitt 2026-09-21

- Ett mastery-faktum räknas nu som självständig kontraktskälla endast om det har spårbart ID och datum, giltig regelversion och minst ett konkret observations-ID.
- Legacyfakta utan dessa belägg bevaras i append-only-historiken men hålls utanför progression, belagd nivå och faktabaserad lärarstatistik. Lärarsammanfattningen redovisar antal kontraktsgiltiga, legacy- och återkallade fakta.
- Klassificerbara problemloggar fortsätter att kunna belägga mastery genom omräkning. När nya svar passerar regeln får ett refererat faktum för samma kompetens/steg läggas till utan att legacyfaktumet raderas eller blockerar uppgraderingen.
- Eventvägen och helprofilens merge bevarar båda posterna per faktum-ID. Därmed överlever uppgraderingen synk och samtidiga fullprofilsuppladdningar.

Verifiering: fokuserade regressionsfall för beräkning, lärarsammanfattning, eventpersistens och servermerge passerade 4 testfiler/41 tester. Hela `npm run test` passerade 73 testfiler/335 tester och `npm run build` passerade med de befintliga Browserslist-/bundlevarningarna. Avgränsad ESLint passerade för övriga ändrade kod- och testfiler; `api/studentStore.test.js` har två redan incheckade `no-useless-escape`-fel på rad 157 utanför det nya testfallet och lämnades orörd där. Ingen data har migrerats, raderats, pushats eller deployats.

### E5 — lokalt verifierat tredje kodsnitt 2026-09-21

- Domänkontraktet kräver nu `verifyContent(problem)`. Verifieraren räknar från strukturerade operander eller uttrycket och använder inte generatorns `answer` som facitkälla.
- Aritmetik, prioriteringsregler, bråk, algebra och procent har separata domänägda kontrollvägar. Procentuppgifter bär nu strukturerad beräkningstyp och operander vid sidan av visningstexten.
- NCM-vägen skiljer `independent_expression` från `external_facit`: strukturerade uttryck räknas om, medan textuppgifter måste matcha kod, uppgiftsnummer, prompt, facit och diagnos-/facitproveniens i den frysta högkonfidensbanken.
- Körningsguardian anropar kontrollen före visning. Ett förfalskat facit kasseras, nästa kandidat provas inom samma begränsade retry-policy och bestående fel går till sessionsfelet utan att bli elevobservation.
- Facitgrinden kör varje registrerad kompetens på varje nivå 1–12 och har negativa fall för avsiktligt felaktiga svar i samtliga kompetenser och steg.
- Variationsgrinden samplar 24 uppgifter per registrerad kompetens/steg och kräver minst åtta unika matematiska promptar. Synliga undantag speglar avsiktligt smala startytor: algebra förenkling nivå 1 kräver tre eftersom ytan är fyra variabler, medan multiplikation och division nivå 1 kräver sex inom sina begränsade grundmängder.

Verifiering: `npm run test` passerade 73 testfiler/338 tester, avgränsad ESLint för samtliga ändrade kod- och testfiler passerade och `npm run build` passerade med de befintliga Browserslist-/bundlevarningarna. Variationsmatrisen passerade dessutom tio fristående upprepningar efter att de smala startytorna gjorts explicita. Ingen autentiserad browser-, iPad-, live-, klassrums- eller produktionsverifiering gjordes.
