...
| Teacher account session | teacher-login -> signed token -> getLiveTeacherAuthPayload | Every account-token protected API route | A signed account token must match a live account and its current sessionVersion. Password, role or direct account-class changes increment that version; deleted/disabled accounts are rejected immediately. Legacy environment-password auth remains a separate compatibility path. |
