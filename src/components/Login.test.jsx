...
vi.mock('../lib/studentLoginClient', () => ({ resolveClassStudentId: mocks.resolve }))
...
describe('login identity wiring', () => {
  it('resolves class and name before authenticating the canonical pupil ID', async () => {
    renderToStaticMarkup(<Login />)
    await mocks.form.onLogin({ classId: '6a-id', name: 'Anna', password: 'Anna' })
    expect(mocks.resolve).toHaveBeenCalledWith('6a-id', 'Anna', 'Anna', '')
  ...
  it('keeps direct ID login available', async () => {
    renderToStaticMarkup(<Login />)
    await mocks.form.onLogin({ classId: '', name: ' anna_a1b2c3 ', password: 'Anna' })
    expect(mocks.resolve).not.toHaveBeenCalled()
    expect(mocks.authenticate).toHaveBeenCalledWith('ANNA_A1B2C3', 'Anna')
  })
  it('does not attempt profile login when class lookup fails or is ambiguous', async () => {
