  ...
  return (
    <form onSubmit={event => {
      ...
      <div>
        ...
        <input id="pilotStudentId" className={inputClass} value={studentId} required maxLength={32}
          onChange={event => { setStudentId(event.target.value); onClearError() }}
          placeholder="Koden på ditt kort" autoComplete="username" disabled={busy} />
      </div>
      <div>
        <label htmlFor="pilotQrSecret" className="block text-sm font-medium text-gray-700 mb-2">QR-hemlighet</label>
        <input id="pilotQrSecret" className={inputClass} value={qrSecret} required maxLength={100}
      ...
      <div>
        ...
        <input id="pilotPin" inputMode="numeric" pattern="[0-9]{4}" className={inputClass} value={pin} required maxLength={4}
