...
const LEVEL_KINDS = {
  geometry_2d_objects: ['line_primitive', 'polygon_sides', 'shape_identity', 'quadrilateral_relation', 'circle_part', 'quadrilateral_properties'],
...
export function verifyGeometryContent(problem) {
  ...
  if (!Number.isInteger(level) || LEVEL_KINDS[problem?.skill]?.[level - 1] !== kind) return { valid: false, reason: 'unexpected geometry level or question type' }
  ...
  if (kind === 'quadrilateral_relation' && problem?.display?.text !== `Är varje ${SHAPES_2D[values.subject.from].label} också en ${SHAPES_2D[values.subject.to].label}?`) return { valid: false, reason: 'relation text disagrees with subject' }
  if (kind === 'quadrilateral_properties' && !String(problem?.display?.text || '').endsWith('Vilken fyrhörning stämmer exakt med egenskaperna?')) return { valid: false, reason: 'quadrilateral prompt disagrees with subject' }
  if (kind === 'solid_identity' && !String(problem?.display?.text || '').endsWith('Välj det mest precisa namnet på kroppen.')) return { valid: false, reason: 'solid prompt is ambiguous' }
  if (kind === 'solid_count') {
    const labels = { faces: 'ytor', edges: 'kanter', vertices: 'hörn' }
    if (problem?.display?.text !== `Hur många ${labels[values.subject.property]} har en ${SOLIDS[values.subject.kind].label}?`) return { valid: false, reason: 'solid count text disagrees with subject' }
  }
  if (kind === 'solid_face_shapes' && !String(problem?.display?.text || '').endsWith('Välj det mest precisa namnet på kroppen med dessa ytformer.')) return { valid: false, reason: 'solid face prompt disagrees with subject' }
  if (kind === 'solid_clues' && !String(problem?.display?.text || '').endsWith('Välj det mest precisa namnet på kroppen med dessa egenskaper.')) return { valid: false, reason: 'solid clues prompt disagrees with subject' }
