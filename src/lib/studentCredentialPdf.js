...
export async function downloadStudentCredentialPdf(credentials, filename = credentialCardsFilename(credentials)) {
  ...
  valid.forEach((credential, index) => {
    ...
    pdfText(doc, 'Öppna matematik.ximon.se', x + 5, y + 21, CARD_WIDTH_MM - 10, 10, 'bold')
    pdfText(doc, '1. Skriv ditt kodnamn', x + 5, y + 32, CARD_WIDTH_MM - 10, 10)
    pdfText(doc, '2. Skriv din PIN', x + 5, y + 41, CARD_WIDTH_MM - 10, 10)
    pdfText(doc, '3. Tryck Logga in', x + 5, y + 50, CARD_WIDTH_MM - 10, 10)
    pdfText(doc, 'Kodnamn', x + 5, y + 63, CARD_WIDTH_MM - 10, 9)
    pdfText(doc, String(credential.displayAlias || '–'), x + 5, y + 72, CARD_WIDTH_MM - 10, 13, 'bold')
    pdfText(doc, 'PIN: ' + credential.pin, x + 5, y + 84, CARD_WIDTH_MM - 10, 13, 'bold')
